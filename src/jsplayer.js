// benry EventTarget
// maxlength
//THIS CODE IS DEVELOPED BY worldace
//https://github.com/worldace/jsplayer
...
class jsplayer extends HTMLElement {
  ...
  static get observedAttributes() {
    return ["file", "comment", "post", "delay"];
  }
  ...
  $動画_timeupdate(event) {
    ...
    if (!this.$シークポインタ.isDrag) {
      ...
      this.$シークポインタ.style.left =
        this.シークポインタ(this.$動画.currentTime / this.$動画.duration).位置 +
        "px";
    }

    if (
      !this.$動画.paused &&
      !this.$jsplayer.hasAttribute("data-comment_off")
    ) {
  ...
  $動画_volumechange(event) {
    ...
    this.$音量ポインタ.style.left =
      this.音量ポインタ(this.$動画.volume).位置 + "px";
  }
  ...
  $シーク枠_click(event) {
    if (this.$動画.readyState && !this.$シークポインタ.isDrag) {
      this.時間変更(
        this.$動画.duration * this.シークポインタ(event.clientX).割合
      );
    }
  }
  ...
  $シークポインタ_mousedown(event) {
    if (this.$動画.readyState) {
      ...
      document.addEventListener("mouseup", this.シークポインタ操作終了_event, {
        once: true
      });
    }
  }
  ...
  $音量ポインタ_mousedown(event) {
    ...
    document.addEventListener("mouseup", this.音量ポインタ操作終了_event, {
      once: true
    });
  }
  ...
  シークポインタ操作_event(event) {
    ...
    this.$現在時間.textContent = this.時間整形(
      this.$動画.duration * ポインタ.割合
    );
  ...
  時間変更(sec) {
    if (this.$動画.readyState) {
      this.$動画.currentTime = this.limit(
        0,
        Math.floor(sec),
        this.$動画.duration
      );
  ...
  ポインタ(current, $ポインタ) {
    //currentは「割合(0-1)」「クリックされた位置」の2パターンある
    ...
    return {
      位置: this.limit(0, 位置, 横幅),
      割合: this.limit(0, 位置 / 横幅, 1)
    };
  }
  ...
  シークバッファ表示() {
    ...
    for (let i = 0; i < buffer.length; i++) {
      if (buffer.end(i) > this.bufferMax) {
        ...
        this.$シークバー.style.backgroundSize = `${
          (this.bufferMax / this.$動画.duration) * 100
        }% 100%`;
      }
    }
  }
  ...
  コメント描画(data, レーン番号) {
    ...
    el.laneNumber = レーン番号;
    el.style.top =
      レーン番号 * this.コメント設定.レーン高さ +
      this.コメント設定.マージン +
      "px";
    el.style.fontSize = this.コメント設定.文字サイズ + "px";
    el.style.animationDelay = this.コメント遅延(
      data[1],
      this.$動画.currentTime
    );
  ...
  コメント遅延(time, duration) {
    ...
    return delay > 0 ? `${delay.toFixed(3) * this.delay}ms` : "0ms";
  }
...
function benry(self) {
  // https://qiita.com/economist/items/6c923c255f6b4b7bbf84
...
customElements.get("js-player") || customElements.define("js-player", jsplayer);
